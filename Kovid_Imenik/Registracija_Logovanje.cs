...
namespace Kovid_Imenik
{
    public partial class Registracija_Logovanje : Form
    {
        ...
        private string HashPassword(string sifra)
        ...
        private bool PasswordsMatch(string pokusajKorisnika, string sacuvanaSifra)
        { // Funkcija koja proverava da li je korisnikov pokusaj tacan password
            string hashedPokusajKorisnika = HashPassword(pokusajKorisnika); // Hash korisnicki unos da bi ga proverili sa onim koji je sacuvan u bazi
            bool doPasswordsMatch = string.Equals(hashedPokusajKorisnika, sacuvanaSifra); // proverava obe sifre
            return doPasswordsMatch; // Vraca rezultat uporedjivanja oba hesirana niza, true ako sifra postoji i false ako ne postoji
        }
        ...
        private void buttonPrijaviSe_Click(object sender, EventArgs e)
        {
            ...
            MySqlCommand command = new MySqlCommand(" SELECT * FROM tabela.korisnici WHERE KorIme = @KorIme AND SHA256Hash = @SHA256Hash",bp.getConnection);
            ...
            command.Parameters.Add("@SHA256Hash", MySqlDbType.VarChar).Value = HashPassword(textBoxSifra.Text);
        ...
        private void buttonRegistrujSe_Click(object sender, EventArgs e)
        {
            ...
            int UporediPolja = Uporedi(textBoxSifraR.ToString(),textBoxSifraPonovo.ToString());
            bool PoljaSifraSuJednaka;

            if(UporediPolja == 0)
            {
                PoljaSifraSuJednaka = true;
            }
            else
            {
                PoljaSifraSuJednaka = false;
            }

            Korisnik korisnik = new Korisnik();

            if (verifFields("Registruj se") & PoljaSifraSuJednaka)
            {
               ...
               if (!korisnik.korImePostojanje(korIme,"Registruj se"))//ako ne postoji
                {
                    if (korisnik.ubaciKorisnika(ime, prezime, korIme, sifra, sifraBol) & PoljaSifraSuJednaka)
                    {
            ...
            {
                MessageBox.Show("Registracija nije uspela, ili sva polja nisu popunjena ili se sifra ne poklapa sa ponovljenom", "Registruj se", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
